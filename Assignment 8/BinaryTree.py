...
class BinaryTree:
    ...
    def insert(self, new_node):
        if new_node < self.get_root_val() and self.left_child == None:
            self.left_child = BinaryTree(new_node)
            

        elif new_node > self.get_root_val() and self.right_child == None:
            self.right_child = BinaryTree(new_node)
            
        
        elif new_node < self.get_root_val():
            self.get_left_child().insert(new_node)

        elif new_node > self.get_root_val():
            self.get_right_child().insert(new_node)
